...
imgGameOver = cv2.imread("Resources/newGameOver.png") #1280 x 720
...
def show_login_page():
    ...
    while not (one_player or two_player or challenge_mode):
        ...
        def mouse_click(event, x, y, flags, param):
            global one_player, two_player, challenge_mode, randomImgEndPhoto
            if event == cv2.EVENT_LBUTTONDOWN:
                randomImgEndPhoto = random.randrange(len(imgEndPhoto))
                if not (one_player or two_player or challenge_mode):
...
while True:
    ...
    if challenge_mode:
        ...
        if gameOver1 and gameOver2:
            img = imgEndPhoto[randomImgEndPhoto]
